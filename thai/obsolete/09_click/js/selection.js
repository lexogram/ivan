;(function selection(){
  ...
  var box = document.body // querySelector("[lang=th]")
  var wordsCache = {}
  var wordsMap = { th: {}, enx: {} }
  ...
  function selectHyphenatedWords(event) {
    ...
    function selectByRegex() {
      var startOffset = Math.max(0, range.startOffset - 1)
      var endOffset = Math.min(range.endOffset + 1, string.length)
      if (string.substring(startOffset, endOffset)
                .search(edgeRegex) < 0) {
