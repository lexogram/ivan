;(function ready() {
  ...
  function initialize() {
    ...
    document.body.addEventListener("transitionend", debouncePlayersTurn);
    start.onclick = startNewGame;
    startNewGame();
  }

  function debouncePlayersTurn(event) {
    if (timeout) {
      return
    }
  ...
  function toggleTemporaryClasses(row) {
    var matchesDivs = document.getElementsByClassName("ignored");
